...
(function ($) {
    ...
    const bgInit = ($this, val = 0, min = 0, max = 255, color = '#f00') => {
        ...
        $this.css('background', `linear-gradient(to right, tomato 0%, #50a2a2 ${valBg}%, #b4b4b4 ${valBg}%, #b4b4b4 100%)`);
    };

    // Pre Init
    const preInit = () => {
        const rangeSliders = ['range-slider-red', 'range-slider-green', 'range-slider-blue', 'range-slider-last'];
